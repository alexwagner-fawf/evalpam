...
$(document).on('shiny:connected', async function() {
  ...
  Shiny.addCustomMessageHandler('ws_load', function(msg) {
    ...
    ws.on('ready', function() {
      ...
      // Padding regions on waveform
      if (msg.detection_start != null && msg.detection_end != null) {
        if (msg.detection_start > 0) {
          regionsPlugin.addRegion({
            start: 0, end: msg.detection_start,
            color: 'rgba(100,100,100,0.15)', drag: false, resize: false
          });
        }
        if (msg.detection_end < duration) {
          regionsPlugin.addRegion({
            start: msg.detection_end, end: duration,
            color: 'rgba(100,100,100,0.15)', drag: false, resize: false
          });
        }
      }

      // Detection marker lines on spectrogram
      ...
      // Deferred so the SpectrogramPlugin canvas is fully painted before we
      // measure its dimensions and attach the selection canvas on top.
      setTimeout(function() {
        ...
        // Use CSS pixel dimensions from getBoundingClientRect so that mouse
        // coordinates (CSS px) map 1:1 to canvas buffer coordinates.
        // specCanvas.offsetWidth / .width can be the retina pixel buffer
        // (e.g. 2048) which makes strokes invisible at mouse scale.
        const scRect = specCanvas.getBoundingClientRect();
        sel.width  = Math.round(scRect.width);
        sel.height = Math.round(scRect.height);
        sel.style.cssText =
    ...
    ws.on('interaction',  function() { ws.playPause(); });
  ...
  Shiny.addCustomMessageHandler('ws_play_selection', function(msg) {
    ...
    if (selectionAudioCtx) { try { selectionAudioCtx.close(); } catch(e) {} }
